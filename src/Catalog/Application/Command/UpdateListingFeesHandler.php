...
final class UpdateListingFeesHandler
{
    ...
    public function __invoke(UpdateListingFees $command): void
    {
        $listing = $this->listings->byId(ListingId::fromString($command->listingId));

        $fees = [];
        foreach ($command->fees as $row) {
            $fees[] = Fee::of(
        ...
        $listing->updateFees($fees, $this->clock);
