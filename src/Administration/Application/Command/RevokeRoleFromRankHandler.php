...
final class RevokeRoleFromRankHandler
{
    ...
    public function __invoke(RevokeRoleFromRank $command): void
    {
        $rank = $this->ranks->byId(RankId::fromString($command->rankId));
        $actor = $this->actors->fromPrimitives($command->actorKind, $command->actorId);

        $rank->revokeRole(RoleId::fromString($command->roleId), $actor, $this->clock);
