...
use App\Administration\Infrastructure\Persistence\Doctrine\Read\DirectRoleGrants;
...
/**
 * Drives the real Doctrine DBAL {@see DirectRoleGrants} source: an
 * administrator holds a privilege through a role assigned directly to
 * them, bypassing their rank — the case LRA-268 describes where a rank
 * is too coarse. Mirrors {@see RankRoleGrantsTest}, assigning the role to
 * the administrator directly rather than to their rank.
 */
#[Medium]
final class DirectRoleGrantsTest extends KernelTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $rank = Rank::define(
            ...
            RankName::of('Direct Grants Fixture Rank'),
        ...
        $this->administrator = Administrator::grant(
        ...
        $this->administrator->releaseEvents();
        $this->administrators->add($this->administrator);
    }

    protected function source(): PrivilegeGrantSource
    {
        $source = static::getContainer()->get(DirectRoleGrants::class);
        self::assertInstanceOf(DirectRoleGrants::class, $source);
    ...
    protected function grantPrivilege(Privilege $privilege, string $sourceId, string $sourceName): void
    {
        ...
        $this->administrator->assignRole(RoleId::fromString($sourceId), Actor::system(), $this->clock);
        $this->administrators->save($this->administrator);
        $this->administrator->releaseEvents();
    }
}
